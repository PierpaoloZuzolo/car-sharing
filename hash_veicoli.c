...
/*
 ...
 Implementazione:
    - Usa ottieni_valori_hash per ottenere tutti i puntatori ai veicoli nella tabella hash.
    - Per ciascun veicolo, aggiorna lo stato della prenotazione tramite aggiorna_prenotazione_veicolo.
    - Verifica se il veicolo è disponibile tramite veicolo_disponibile.
    - Se disponibile, chiama stampa_veicolo per visualizzarlo.
    - Libera la memoria allocata da ottieni_valori_hash prima di terminare.
...
void stampa_veicoli_disponibili(ptr_hash_veicoli h)
{
    ...
    if (!valori || numero_elementi == 0) {
        printf("Nessun veicolo presente nella tabella hash.\n");
        return;
    }
